...
def load_data(csv_file_path):
    ...
    # Pre-filtrar por la Provincia de Llanquihue
...
# --- Carga de FORMAS (SHAPEFILE) ---
@st.cache_data
def load_shapes(shapefile_path):
    try:
        gdf = gpd.read_file(shapefile_path)
        
        columna_nombre_comuna = ''
        if 'Comuna' in gdf.columns:
            columna_nombre_comuna = 'Comuna'
        elif 'NOM_COMUNA' in gdf.columns:
            columna_nombre_comuna = 'NOM_COMUNA'
        else:
            st.warning("No se pudo encontrar la columna de nombre de comuna en el Shapefile. Los bordes no se mostrarán.")
            return None
            
        gdf['Comuna_Corregida'] = gdf[columna_nombre_comuna].replace(COMUNA_MAPPING)
        gdf_llanquihue = gdf[gdf['Comuna_Corregida'].isin(COMUNAS_LLANQUIHUE)].copy()
        gdf_llanquihue = gdf_llanquihue.to_crs(epsg=4326)
        
        return gdf_llanquihue
        
    except Exception as e:
        st.error(f"Error al cargar el archivo Shapefile ('{shapefile_path}'): {e}")
        st.info("Asegúrate de que el archivo .shp (y sus .dbf, .shx, .prj) estén en la misma carpeta que app.py")
        return None

# --- Cargar los datos ---
DATA_FILE = 'BASE TRABAJO FINAL.xlsx - Sheet1.csv'
SHAPE_FILE = 'comunas.shp'
...
sexo_seleccionado = st.sidebar.multiselect(
    ...
    default=sexo_disponible  # <-- ¡AQUÍ ESTÁ LA CORRECCIÓN!
)
